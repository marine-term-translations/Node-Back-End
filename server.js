...
app.post("/api/github/token", async (req, res) => {
  ...
  if (!client_id || !client_secret) {
    ...
    return res.status(500).json({
      error: "Internal Server Error",
      message:
        "GitHub Client ID or Client Secret is missing in the environment variables.",
    });
  }
...
app.get("/api/github/branches", async (req, res) => {
  ...
  try {
    ...
    const branches = response.data.filter((branch) =>
    ...
    const branchDetails = await Promise.all(
      branches.map(async (branch) => {
        try {
          ...
          console.error(
            `Error retrieving commit details for branch ${branch.name}:`,
            commitError
          );
          return {
            name: branch.name,
...
app.get("/api/github/tree", async (req, res) => {
  ...
  if (!repo || !branch) {
    return res.status(400).json({
      error: "Bad Request",
      message:
        'The "owner", "repo", and "branch" query parameters are required.',
    });
  }
  ...
  try {
    ...
    console.error("Error while retrieving the file tree:", error);
    ...
    res.status(500).json({
      error: "Internal Server Error",
      message: "An unexpected error occurred while retrieving the file tree.",
    });
  }
});
...
app.get("/api/github/diff", async (req, res) => {
  ...
  if (!branch.startsWith(process.env.GITHUB_KEY_BRANCH)) {
    return res.status(400).json({
      error: "Bad Request",
      message:
        "Invalid branch name. The branch name must start with the specified key branch prefix.",
    });
  }
    ...
    const conflictslist = await Promise.all(
      commonFiles.map(async (filename) => {
        try {
          ...
          const contentResponseBranch = await octokit.request(
            "GET /repos/{owner}/{repo}/contents/{path}",
            {
              owner,
              repo,
...
app.get("/api/github/content", async (req, res) => {
  ...
  try {
    ...
    res.json(
      parse(Buffer.from(response.data.content, "base64").toString("utf-8"))
    );
  } catch (error) {
...
app.put("/api/github/update", async (req, res) => {
  ...
  if (!repo || !translations || !branch || !filename) {
    return res.status(400).json({
      error: "Bad Request",
      message:
        'The "repo", "translations", "filename", and "branch" fields are required in the request body.',
    });
  }
  ...
  try {
    ...
    const responseSha = await octokit.request(
      "GET /repos/{owner}/{repo}/contents/{path}",
      {
        owner,
        repo,
...
app.get("/api/github/changed", async (req, res) => {
  ...
  try {
    ...
    if (!pullsResponse.data.length) {
      ...
      const createPullResponse = await octokit.request(
    ...
    const diffsData = await Promise.all(
      files.map(async (file) => {
        try {
          ...
          const beforeResponse = await octokit.request(
            "GET /repos/{owner}/{repo}/contents/{path}",
            {
              owner,
              repo,
          ...
          const afterResponse = await octokit.request(
            "GET /repos/{owner}/{repo}/contents/{path}",
            {
              owner,
              repo,
    ...
    res.status(500).json({
      error: "Internal Server Error",
      message:
        "An unexpected error occurred while retrieving the changed files or comments.",
    });
  }
});
...
app.put("/api/github/merge", async (req, res) => {
  ...
  if (!repo || !branch) {
    return res.status(400).json({
      error: "Bad Request",
      message:
        'Both "repo" and "branch" fields are required in the request body.',
    });
  }
  ...
  try {
    ...
    const pullDetailsResponse = await octokit.request(
...
app.post("/api/github/comment", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send(
        `CLIENT_ID = ${CLIENT_ID} or ${process.env.GITHUB_CLIENT_ID} Server internal error`
      );
  }
});
